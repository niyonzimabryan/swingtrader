...
> Parking lot for forward-looking features / directions. Includes items deprioritized in the 2026-05-12 Hermes-kanban audit (those tasks are now priority-1 with a back-reference comment — task IDs noted below). The active near-term backlog lives in the kanban / Linear.

**Data-gated (need closed-trade volume that doesn't exist yet)**
- [ ] **Tune scoring weights from real data** — After 50+ closed trades, run attribution analysis to see which agents actually predict winners, then rebalance weights. `tracking/attribution.py` is currently a stub. _(kanban t_a2f179ce)_
- [ ] **Pattern Agent: incorporate own trade history** — Once 30+ closed trades exist, add our own trade outcomes as pattern data alongside historical market data (higher-signal — they went through the full pipeline). _(kanban t_2defc77f)_
- [ ] **RL / training loop for scoring** — RL or fine-tuning on pattern data + trade outcomes; closed-trade P&L as reward to optimize weights / prompts / classification. Start simple (Bayesian weight optimization from attribution) → graduate. _(kanban t_aaf7df8f)_

**Features**
- [ ] **Backtest framework** — PRD Phase 2: replay historical data through the pipeline to validate strategy before going live. `backtest/` is currently an empty stub. _(kanban t_46479d20)_
- [ ] **Email backup channel** — PRD calls for email delivery as a Telegram backup. Not MVP-critical; useful for an audit trail. _(kanban t_c0893182)_
- [ ] **Portfolio rebalancing** — Auto-suggest trimming winners / adding to conviction positions based on drift from target allocation. _(kanban t_08deab78)_
- [ ] **Batch approval flow for scheduled scans** — Scheduled scans generate memos but there's no batch-review UX; queue them for a morning review? _(kanban t_5c29e2dc)_
- [ ] **User override of position size** — Let the operator override the system's calculated size, or keep it authoritative? Open design question. _(kanban t_a0b6ced4)_
- [ ] **Opus-adjusted WATCHLIST/PASS params** — When Opus says watchlist/pass, have it generate reduced-size / tighter-stop "if you must trade" params instead of showing Sonnet's raw draft. _(kanban t_d4bcd297 — relabel-only shipped in PR #2; the Opus-generates-params half is still active near-term work, not just a future idea)_
- [ ] **Multi-timeframe analysis** — Current system is swing-focused (3-15 day). Could add day-trade and position-trade modes.

**Perf / refactor**
- [ ] **Cache-first reads for repeat analyses** — Optional optimization; only worth it if repeated same-day ticker analysis becomes common. _(kanban t_3dea0021)_
- [ ] **Native-async pipeline** — `run_ad_hoc_async` uses `loop.run_in_executor`; could make the full pipeline natively async. Not flagged by `ARCHITECTURE_EVOLUTION_TRIGGERS.md` — premature for now. _(kanban t_df2943b8)_

**Done (kept here for context)**
- [x] **Expand ticker universe** — ✅ S&P 500 (503 tickers, 11 GICS sectors), auto-generated via `scripts/update_sp500.py`. Could later add mid-caps or a dynamic screener-based refresh.
- [x] **Dockerfile for deployment** — ✅ Done. Dockerfile + railway.toml deployed to Railway.
- [x] **Watchlist with alerts** — ✅ Full stack (CRUD backend in `orchestrator/universe.py`, lower Haiku re-scan threshold, "Watchlist" button on memos, `/watchlist` command with inline remove). Remaining idea: dedicated alert notifications when catalysts strengthen between scans.
