...
**Constraints:** read paths only — no order placement code in this phase; first checkpoint records which Robinhood MCP server is connected (official agent.robinhood.com vs an unofficial community server), its tool list and auth model, in docs/ROBINHOOD_INTEGRATION_PLAN.md with evidence, and stops for an owner decision if it is unofficial; sync is append-only and never zeroes a position on error; null cost basis stays null; option positions are stored with instrument_type and rendered as unsupported_instrument_present, never omitted; hourly sync cadence; no Schwab adapter (owner deferred it) — ship the BrokerCapabilities contract, a fake broker, and the contract tests instead; PORTFOLIO_SYNC_ENABLED defaults false
**Validate:** `python -m unittest discover -s tests -p "test_*.py" && python -m compileall -q .` after each change
**Document:** Write concise, targeted documentation for all changes — create new .md files or update existing docs as needed.
...
**Read first:** specs/investment-workspace/N-comparable-setups-engine.md in full, specs/investment-workspace/research/2026-09-05-best-in-class-research.md sections D and G, backtest/simulator.py, backtest/event_replay.py, data/analog_ranker.py, data/event_outcomes.py
**Constraints:** reuse backtest/simulator.py for policy-simulated outcomes — do not reimplement exit semantics; add `arch` and use `statsmodels` (already transitively present — pin it explicitly) for inference, never `mlfinlab`; implement `depth="quick"` first and get a real answer before building `full`; no field in CohortAnswer is optional at its depth; `insufficient` is a valid return and must never be softened into a hedge; no model output may become a number; do not tune a setup to make a cohort look better; the lookahead harness (test_truncated_data_identical_answer) runs against every stored cohort in CI; universe_membership is seeded from the MIT fja05680/sp500 history and the existing event library, with the tier capped honestly where delisted names are missing; COMPARABLE_SETUPS_ENABLED defaults false
**Validate:** `python -m unittest discover -s tests -p "test_*.py" && python -m compileall -q .` after each change
**Document:** Write concise, targeted documentation for all changes — create new .md files or update existing docs as needed.
...
**Read first:** specs/investment-workspace/O-evidence-planes.md, specs/investment-workspace/research/2026-09-05-best-in-class-research.md slots 3-6, data/macro_data.py, data/news_data.py, data/sec_data.py, specs/investment-workspace/strategy-lab/strategy-lab-architecture.md section 8
**Constraints:** every fact carries valid_at, known_at_utc, precision, and a provenance class; a guessed known_at_utc means replay_eligible=false; day-precision facts are known at the close of their date; use edgartools + SEC bulk data sets + OpenFIGI for filings and the SEC XBRL companyfacts/submissions join for fundamentals; use the Alpaca News API as the primary news source and remove Gemini-search/Firecrawl from any cohort-eligible path; no LLM in the regime classifier, and regime_v1 uses only never-revised inputs; 13F positions never render without staleness and portfolio share; Form 4 transaction codes P/S are never pooled with A/M/F/G; respect source rate limits in one client module; each plane flag defaults false
**Validate:** `python -m unittest discover -s tests -p "test_*.py" && python -m compileall -q .` after each change
**Document:** Write concise, targeted documentation for all changes — create new .md files or update existing docs as needed.
...
Do not dispatch until Phases 0–3 are merged, Robinhood's protective-exit capability is
documented with evidence, and the README §3 Robinhood execution-scope decision is made
(the official server places orders only in a separately funded Agentic account). The order proposal, approval, and execution lifecycle is
