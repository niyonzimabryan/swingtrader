...
## Phase 3a — Minimum SEC ingestion contract (blocks 3b; free, no credentials)

```
**Objective:** Land filings/sec_minimal.py so that SEC XBRL companyfacts, the submissions acceptanceDateTime per filing, the dei:EntityCommonStockSharesOutstanding share count, and the 8-K Item 2.02 index write point-in-time rows into source_observations, per specs/investment-workspace/N-comparable-setups-engine.md section 4.0 and specs/investment-workspace/O-evidence-planes.md sections 2 and 3.4.
**Read first:** specs/investment-workspace/O-evidence-planes.md sections 2 and 3.4, specs/investment-workspace/N-comparable-setups-engine.md section 4.0, docs/research/2026-09-research-verification.md claim 13, data/sec_data.py
**Constraints:** requires Phase 0a merged; known_at_utc is acceptanceDateTime and a test rejects filingDate; every row carries precision and provenance class; explicit XBRL tag-alias maps with a coverage-discontinuity alert; SEC rate limit 10 req/s with an identifying User-Agent, throttled in one client module; no Phase 4 planes (13D/G, Form 4, news) in this phase; an adapter receiving an unexpected payload shape raises and never writes nulls; PLANE_SEC_MINIMAL_ENABLED defaults false
**Validate:** `python -m unittest discover -s tests -p "test_*.py" && python -m compileall -q .` after each change
**Document:** Write concise, targeted documentation for all changes — create new .md files or update existing docs as needed.
**Checkpoints:** work in checkpoints; log coverage (companies with a continuous revenue and EPS series, share-count coverage, 8-K 2.02 hit rate) on a fixed 50-ticker fixture
**Stop when:** test_xbrl_known_at_from_acceptance, test_filing_date_never_known_at, test_xbrl_alias_coverage_alert, test_8k_202_timestamp, test_market_cap_has_share_source, and test_adapter_schema_change_fails_loudly pass, and a real run against the 50-ticker fixture reports its coverage — OR when an XBRL normalisation decision needs human judgment.
```

## Phase 3b — Measure it (the centerpiece)
