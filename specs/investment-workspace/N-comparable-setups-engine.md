...
- **Open at merge:** a real answer carries `PolicySummary.net` but no lower-90%
  ...
  a real citation yet. Closed by the follow-up ratified
  immediately below (a `net_ci` on the policy leg, a `subject_ticker` recorded
  at query time, and an evidence gate that reads the stored answer).
